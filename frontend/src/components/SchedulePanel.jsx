...
function GeoRow({ item, onSave }) {
  ...
  const INTERVAL_OPTIONS = [
    { label: t("interval_5m"),  value: 0.083 },
    { label: t("interval_1h"),  value: 1 },
    { label: t("interval_6h"),  value: 6 },
    { label: t("interval_12h"), value: 12 },
    { label: t("interval_1d"),  value: 24 },
    { label: t("interval_3d"),  value: 72 },
    { label: t("interval_4d"),  value: 96 },
    { label: t("interval_1w"),  value: 168 },
  ];
    ...
    <div className={`flex flex-wrap items-center gap-4 px-4 py-3 rounded-xl border transition-colors ${
      ...
      <div className="text-xs text-gray-500 space-y-0.5 min-w-[150px]">
        <p className="flex items-center gap-1">
          <Clock size={10} /> {t("last_run")} {fmt(item.last_run, locale)}
        </p>
        <p className="flex items-center gap-1">
          <CalendarClock size={10} /> {t("next_run")} {fmt(item.next_run, locale)}
        </p>
      </div>
      ...
      <label className="flex items-center gap-2 cursor-pointer select-none">
        ...
        <span className="text-xs text-gray-400 w-16">
          {enabled ? t("enabled_on") : t("enabled_off")}
        </span>
      </label>
      ...
      >
        ...
        {saved ? t("saved_short") : t("save_btn_short")}
      </button>
    ...
    <div className="card space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-gray-300 flex items-center gap-2">
          <CalendarClock size={14} className="text-sky-400" /> {t("schedule_title")}
        </h2>
        <span className="text-xs text-gray-600">{t("schedule_subtitle")}</span>
      </div>
